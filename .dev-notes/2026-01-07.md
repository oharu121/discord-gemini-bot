...
### 6. Failed Health Endpoint Attempts (2026-01-08)

**Goal:** Add `/healthz` endpoint for Google Apps Script to ping and keep HF Space awake.

**Attempt 1: `@demo.app.get("/healthz")`**
- Added endpoint via Gradio's underlying FastAPI app (lines 96-100 above)
- **Result:** Returns Gradio UI instead of "ok"
- **Why it failed:** Gradio's catch-all routing (`/` and `/{page}`) intercepts all undefined routes and serves the HTML interface

**Attempt 2: `gr.api(healthz, api_name="healthz")`**
- Used Gradio's official API endpoint system (added in v5.11)
- Creates endpoint at `/gradio_api/call/healthz`
- **Result:** HTTP 405 (Method Not Allowed) on GET request
- **Why it failed:** Gradio API endpoints require two-step flow:
  1. POST to `/call/<api-name>` to queue request and get event ID
  2. GET to `/call/<api-name>/<event-id>` to retrieve result
- Too complex for simple health check ping

**Lesson Learned:**
Gradio is designed for ML/AI interfaces, not for simple REST APIs. For a lightweight health check:
- **Solution:** Use root `/` URL for health checks - the Gradio interface loading successfully proves the app is healthy
- **Alternative:** If a custom `/healthz` endpoint is truly required, must use FastAPI mount pattern (creates FastAPI app, adds `/healthz`, then mounts Gradio) - but this risks breaking the Discord connection like Docker SDK did

**Decision:** Abandoned custom health endpoint. Google Apps Script will ping root `/` instead.
